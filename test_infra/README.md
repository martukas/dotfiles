...
- **No conceptual dependency management in superpack.** `packages.yml` has no `requires:` / `dependencies:` field, so real soft-deps go undeclared: guake → nerdfonts (terminal font), powershell + oh-my-posh → nerdfonts (theme glyphs), powershell → brew (oh-my-posh source), keepass plugins → keepass2. If the user picks a dependent package without its prereq, install proceeds and the dependent renders/runs incorrectly without a clear error. Could be addressed by extending superpack's schema, or just surfacing the chain in `packages.yml` comments / README so the user can pick correctly. Larger discussion needed.
- **Powershell + Nerd Fonts coupling holds on Linux too.** `common/powershell/blue-owl.omp.json` uses Nerd Font glyphs. If `powershell` is selected without `nerdfonts`, prompts render with missing-glyph boxes — same as Windows, not a Windows-only assumption.
- **`install_nerdfonts` pins v2.1.0 (2021).** Latest Nerd Fonts release is 3.x. Bump next time the function is touched — newer glyphs, better OS coverage.
- **xfce4-screensaver hangs on 26.04 after extended idle.** After the screensaver triggers on inactivity and runs for some time, the controller (`xfce4-screensaver` daemon) stops responding on DBus (`xfce4-screensaver-command --query` times out). The floaters animation child keeps running fine, but input events don't reach the unlock path — spice display appears frozen, no keyboard/mouse recovery. Workaround: `pkill -9 xfce4-screensaver` over SSH; the daemon respawns under xfce4-session and the desktop becomes usable again. Root cause unknown — possible interaction with spice input handling, session DBus, or an xfce4-screensaver bug specific to 26.04. Observed 2026-04-30 in `gui26` after ~34 min of screensaver activity following a fresh login. If it recurs, candidate mitigations: disable theme rotation (`mode=1`), longer idle delay, or replace with the `xscreensaver` package entirely.
